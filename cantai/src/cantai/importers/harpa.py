"""Importador do hinário Harpa Cristã."""
...
_HYMNAL = "HARPA"
_HYMN_PATTERN = re.compile(r'^(\d+)\.\s+["""\']?(.+?)["""\']?$')


def _extract_hymns_from_pdf(pdf_path: Path) -> list[tuple[int, str, str]]:
    """Extrai hinos do PDF da Harpa Cristã.
...
def _parse_lyrics(hymn_text: str, title: str) -> tuple[str, str]:
...
def import_harpa(pdf_path: Path) -> tuple[list[Hymn], AuditReport]:
    """Importa hinário Harpa Cristã de um arquivo PDF.
    ...
    for num, title, hymn_text in raw_hymns:
        ...
        try:
            lyrics, first_line = _parse_lyrics(hymn_text, title)
            if not lyrics:
